...
from app.utils.games_list import build_games_list
...
@router.get("/{project_id}/games-list", response_class=PlainTextResponse)
async def get_games_list(project_id: str, request: Request):
    """Markdown list of games in the project, with embedded store URLs and
    sentence-cased publishers. Auto-fetches missing publishers and persists
    them back into draft state."""
    projects_dir = _projects_dir(request)
    project = await _load_project(projects_dir, project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    storage_root = Path(request.app.state.store.root)
    orchestrator = request.app.state.orchestrator
    return await build_games_list(project, storage_root, orchestrator)
