...
from app.resolvers.game_icon_api import GameIconApiResolver
...
class Orchestrator:
    def __init__(self, store: AssetStore, cache: GameAssetCache | None = None) -> None:
        self._store = store
        self._cache = cache
        self._icon_api = GameIconApiResolver(store, settings.game_icon_api_url)
    ...
    async def update_draft(self, draft: DraftState) -> DraftState:
        ...
        # Re-resolve any inspiration icons that are missing or failed
        await self._refresh_missing_icons(draft)

        await self._render_and_save(draft)
        await self._persist_draft(draft)
        return draft

    async def _refresh_missing_icons(self, draft: DraftState) -> None:
        """
        For each inspiration that has no icon or a failed icon, re-run the full
        resolver chain: iTunes/Play Store → Supabase theme → DALL-E → Iconify → initials.
        Updates the inspiration in-place.
        """
        for idx, insp in enumerate(draft.inspirations):
            already_ok = (
                insp.icon_status == IconStatus.ok
                and insp.icon_asset_key
                and await self._store.exists(insp.icon_asset_key)
            )
            if already_ok:
                continue

            logger.info(
                "Re-resolving icon for inspiration %r (draft %s, idx %d)",
                insp.name, draft.id, idx,
            )
            icon_bytes = await self._icon_api.resolve(insp.name)

            if icon_bytes:
                key = f"drafts/{draft.id}/icon_{idx}.png"
                await self._store.put(key, icon_bytes)
                insp.icon_asset_key = key
                insp.icon_status = IconStatus.ok
                logger.info("Icon resolved for %r → %s", insp.name, key)
            else:
                insp.icon_asset_key = None
                insp.icon_status = IconStatus.needs_upload
                logger.info("Icon NOT resolved for %r — leaving blank", insp.name)
    ...
    async def create_draft_from_text_slide(
        ...
        for i, insp_data in enumerate(inspirations_data):
            ...
            icon_bytes = await self._icon_api.resolve(name)
            if icon_bytes:
    ...
    async def _resolve_icon(self, icon_src, draft_id: uuid.UUID, idx: int):
        if icon_src.source == "auto":
            icon_bytes = await self._icon_api.resolve(icon_src.query)
            if icon_bytes:
            ...
            icon_bytes = await self._icon_api.resolve(icon_src.name)
            if icon_bytes:
                key = f"drafts/{draft_id}/icon_{idx}.png"
                await self._store.put(key, icon_bytes)
                return icon_bytes, key, IconStatus.ok
